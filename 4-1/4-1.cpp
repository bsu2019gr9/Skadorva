...
int main() {
   ...
   //поместит '\0', установит флаг ошибки (failbit), поток станет нерабочим, не введённое останется в очереди ввода.
       ...
       j++;
       count = 0;
        for (int ip = 1; ip < i; ++p) {//---->ещё вопрос: как сделать сравнение строк по числу совпадений. Я пробовал сделать массив указателей, где под номером строки хранилось бы число совпадений и потом вызывать эти значения для сравнения
        ...
        cout << buff << '\n'; //на экран
        ggggg << buff << "***";//в файл  ---> тоже не очень понимаю, что в моей задаче нужно записывать в файл
